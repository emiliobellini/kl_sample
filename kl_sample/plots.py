...
def plots(args):
    ...
    path['mcm'] = io.path_exists_or_error(io.read_param(args.params_file,
                                          'mcm', type='path'))+'/'
    path['fourier'] = io.path_exists_or_error(
        '{}/data/data_fourier.fits'.format(sys.path[0]))
    path['output'] = io.path_exists_or_create(
...
#    sims_EE_kl = rsh.mask_cl(sims_EE_kl, is_diag=True)
    ...
    d = fits.open(
        "/users/groups/damongebellini/data/preliminary/7bins/cls_bf.fits")
    cl_ee = d[2].data[:len(th_cl)]

    # th_cl = rsh.bin_cl(th_cl, bp)
    tot_ell = np.arange(bp[-1, -1]+1)
    th_cl, th_cl_BB = rsh.couple_decouple_cl(
        tot_ell, th_cl, path['mcm'], len(fields), n_bins, len(bp),
        return_BB=True)
    th_clb, th_cl_BBb = rsh.couple_decouple_cl(
        tot_ell, cl_ee, path['mcm'], len(fields), n_bins, len(bp),
        return_BB=True)
    ...
    io.write_to_fits(fname=path['output']+'obs.fits', array=cl_EE_kl,
                     name='cl_EE_obs')
    io.write_to_fits(fname=path['output']+'obs.fits', array=cov_kl,
                     name='cov_obs')
    io.write_to_fits(fname=path['output']+'obs.fits', array=th_cl_kl,
                     name='cl_EE_th')
    plt.figure()
    x = ell
    for b1 in range(3):
...
#        plt.plot(x, y1, label = '$\\sigma_g^2/n_{eff}$')
    ...
    for b1 in range(n_bins):
        for b2 in range(b1, n_bins):
            ...
            plt.savefig(
                '{}noise_bin{}{}.pdf'.format(path['output'], b1+1, b2+1))
            plt.close()
