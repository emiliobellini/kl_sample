...
def prep_fourier(args):
    ...
    n_avg = 5 # Number of new pixels used to average the multiplicative correction to the shear
    ...
    # Main loop to scan over the fields TODO: replace ['W3'] with fields
    for f in ['W3']:
        ...
        # TODO: remove this and uncomment the previous lines
        hd = io.read_header_from_fits(path['input']+'/'+f+'_mask.fits', f+'_mask')
        mask = io.read_from_fits(path['input']+'/'+f+'_mask.fits', f+'_mask')
        w = wcs.WCS(hd)
        z_bins = np.array([z_bins[0]]) #TODO: remove this


        # Calculating the map
        print 'Calculating map for field ' + f + ':'
        sys.stdout.flush()
    ...
    # c2 = gals['c2'][0]
    # zb = gals['Z_B'][0]
    # p1 = gals['ALPHA_J2000'][0]
    # p2 = gals['DELTA_J2000'][0]
    # weight = gals['weight'][0]
